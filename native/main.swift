...
MainActor.assumeIsolated {
    ...
    let uiTest = CommandLine.arguments.contains("--self-test-ui")
    let selfTest = CommandLine.arguments.contains("--self-test") || uiTest
    #else
    let selfTest = false
    #endif
    ...
    #if LITEBAR_VALIDATION
    if selfTest { controller.onValidation = { value in _ = Task { @MainActor in await validateNative(value, liveInput: !uiTest) } } }
    #endif
